...
// ['bagmati', 'sudurpaschim', 'lumbini', 'pradesh-1', 'madhesh', 'gandaki', 'karnali']
...
// Search with options
const exactMatch = searchDistricts('kathmandu', undefined, { exactMatch: true });
// ['kathmandu']
...
const municipalitySuggestions = searchMunicipalities(userInput, selectedDistrict);
```
...
function handleFormSubmit(formData: { province: string; district: string; municipality: string }) {
  if (validateAddress(formData.province, formData.district, formData.municipality)) {
